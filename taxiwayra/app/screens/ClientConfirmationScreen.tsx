...
import { Alert, Dimensions, ImageStyle, View, ViewStyle, Vibration, TextStyle } from "react-native"
...
export const ClientConfirmationScreen: FC<ClientConfirmationScreenProps> = observer(
  function ClientConfirmationScreen(_props) {
    ...
    const [socioId, setSocioId] = useState<any | null>(null)
    ...
    useEffect(() => {
      socket.on("socio-events-change", (res: { socio_id: string; data: any }) => {
        ...
        if (viajeIdConfirmado === res?.data?.id) {
          ...
          setConfirmado(true)
          setLoadingBtn(false)
          // seteamos value para buscar al socio
          findUserById(res.socio_id).then()
          socket.emit("socio_join", res.socio_id)
    ...
    const findUserById = async (socioId: string) => {
      console.log(socioId)
      socioService
        .getById(socioId)
        .then((res: any) => {
        ...
        {socio && (
          ...
          >
            {socio && socio?.veiculo ? (
              <AutoImage
                ...
                source={{
                  uri: urlImgServer(
                    `public/socios-files/${
                      socio?.veiculo?.foto ? socio?.veiculo?.foto : socio?.foto
                    }`,
                  ),
                  // uri: "https://www.seguridadvialenlaempresa.com/media/blog/taxista-seguridad-pasajeros-1194x585-1.jpg",
                }}
              />
            ) : (
              <AutoImage
            ...
            <View style={{}}>
              <Text
                preset="default"
                style={[$text, { color: colors.palette.accent500, fontSize: 12 }]}
              >
                {/* <Icon icon="user" color={colors.tint} size={20} />{" "} */}
                {`${socio?.veiculo ? socio?.veiculo?.placa : "S/N"} `}
              </Text>
              <Text preset="default" style={[$text, { alignItems: "center" }]}>
                {/* <Icon icon="userTeam" color={colors.tint} size={20} />{" "} */}
                {`N: ${socio?.veiculo ? socio?.veiculo?.n_movil : "S/N"} `}
              </Text>
            </View>
          </View>
        )}
        ...
        {confirmado === false && origin && destination ? (
          <View style={$buttonContainer}>
            <Button
              ...
              testID="registeras"
              text={loadingBtn ? "Esperando..." : "Solicitar vehículo"}
          ...
          <View style={$buttonContainer}>
            <Button
              ...
              testID="registeras"
              text={"Ver ubicacion del vehículo"}
              style={$tapButtonInitRoute}
              preset="reversed"
              disabled={loadingBtn}
